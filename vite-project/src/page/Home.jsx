...
return (
    <div className='min-h-screen bg-gray-100 p-6'>
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 mb:grid-cols-3 lg:grid-cols-cols-4 gap-6'>
            {data.map((item) => (

                <div key={item.id} 
                    className='bg-white shadow-lg shadow-lg rounded-xl p-4 hover:shadow-xl transition'>
                    <img src={item.thumbnail} alt={item.title} className='h-40 w-full object-contain mb-3' />
                    <h3 className='font-semlbold text-lg'>{item.title}</h3>
                    ...
                    <div className='mt-3 flex justify-between items-center'>
                        ...
                        <Link to={`/product/${item.id}`}>
                            <button className='px-4 py-1 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition'>View</button>
                        </Link>
                    </div>
                </div>
